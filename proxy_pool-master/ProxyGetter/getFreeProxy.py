...
class GetFreeProxy(object):
    ...
    # 自定义代理
    @staticmethod
    def GetWanDouProxy():  # 豌豆代理
        url = "http://api.wandoudl.com/api/ip?app_key=857dbfad86b6be85dcb42af19ad2ecd3&pack=2054&num=100&xy=1&type=2&mr=1&"
        new_ip = [f'{i.get("ip")}:{i.get("port")}' for i in requests.get(url).json().get('data')]
        for i in new_ip:
            yield i
